...
Daily quote from a bundled offline pack. Click the cited author to
open their Wikipedia page.

### Photo

Daily photo from Picsum (no key) or NASA APOD (your free DEMO_KEY or
personal API key). On APOD video days, renders the official
`thumbnail_url` with a play-badge overlay.
